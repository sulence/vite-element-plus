...
import { PAGE_NOT_FOUND_ROUTE, REDIRECT_ROUTE } from "@/router/routes/base";
...
function handleGotoPage(route: RouteLocationNormalized, router: Router) {
  const go = useGo(router);
  go(unref(route).path, true);
}
...
export const useMultipleTabStore = defineStore({
  ...
  actions: {
     ...
     * Refresh tabs
     */
    async refreshPage(route: RouteLocationNormalized, router: Router) {
      ...
      const redo = useRedo(route, router);
      await redo();
    },
    clearCacheTabs(): void {
